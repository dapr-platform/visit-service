...
func StartCamLiveStream(cameraID string, disableSaveMp4 bool) (string, string, error) {
	// 1. 获取摄像头信息
	...
	streamPath := fmt.Sprintf("/playback/%s/%s/%s", time.Now().Format("2006-01-02"), camera.ID, streamID)
	...
	// 3. 返回流ID
	return streamID, streamPath, nil
}
